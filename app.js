...
const MESSAGE_KEY='brinky_messages_v1';
const COMPANY = {
  name:'BRINKY FIESTA',
...
function renderDashboard(){
  ...
  $('statContracts').textContent=items.length;$('statUpcoming').textContent=upcoming.length;$('statIncome').textContent=money(items.reduce((a,d)=>a+Number(d.total||0),0));if($('statMembers'))$('statMembers').textContent=getLoyalty().length;if($('statMessages'))$('statMessages').textContent=getMessages().filter(m=>m.status==='pending').length;
...
function getMessages(){try{return JSON.parse(localStorage.getItem(MESSAGE_KEY))||[]}catch{return []}}
function setMessages(items){localStorage.setItem(MESSAGE_KEY,JSON.stringify(items));renderMessages();refreshMessageStats()}
function messageTypeLabel(type){return ({welcome:'Bienvenida',update:'Actualización',reward:'Premio desbloqueado',referral:'Referido'})[type]||'Mensaje'}
function welcomeMessage(c){const s=getLoyaltySettings();return `🎉 *¡Bienvenido al Club Brinky Fiesta!* 🐸\n\n¡Hola *${c.name}*! Tu registro fue completado con éxito.\n\nComo agradecimiento por unirte, recibiste tu primera *⭐ Estrella Brinky de Bienvenida*.\n\n🌟 *Tu progreso:*\n${'⭐'.repeat(Math.min(Number(c.stamps||0),s.r2))}${'⬜'.repeat(Math.max(0,s.r2-Number(c.stamps||0)))} (${c.stamps} de ${s.r2})\n\n🎁 *Recompensas:*\n⭐ ${s.r1} Estrellas = *${s.n1}*\n🏆 ${s.r2} Estrellas = *${s.n2}*\n\n🤝 Recomienda a un amigo usando tu código de referencia *${c.code}* y, cuando realice su renta, ambos ganan 1 Estrella Brinky.\n\n💙 ¡Gracias por formar parte del Club Brinky Fiesta!`}
function updateMessage(c,reason=''){const s=getLoyaltySettings(),r=rewardStatus(c);return `⭐ *¡Tu Tarjeta Club Brinky fue actualizada!*\n\n¡Hola *${c.name}*! ${reason?reason+'\n\n':''}Ahora tienes *${c.stamps} de ${s.r2} Estrellas Brinky*.\n\n${'⭐'.repeat(Math.min(Number(c.stamps||0),s.r2))}${'⬜'.repeat(Math.max(0,s.r2-Number(c.stamps||0)))}\n\n🎁 ${s.r1} Estrellas = *${s.n1}*\n🏆 ${s.r2} Estrellas = *${s.n2}*${r?`\n\n🎉 *¡Felicidades! Ya puedes canjear: ${r.name}*`:''}\n\n🤝 Comparte tu código *${c.code}*. Cuando un amigo rente usando tu código, ambos ganan 1 Estrella Brinky.\n\n💙 ¡Gracias por elegir Brinky Fiesta!`}
function referralMessage(c,friendName='un amigo'){return `🤝 *¡Ganaste una Estrella Brinky por recomendación!* ⭐\n\n¡Hola *${c.name}*! Gracias por recomendar Brinky Fiesta a *${friendName}*.\n\nTu tarjeta ahora tiene *${c.stamps} Estrellas Brinky*.\n\nSigue compartiendo tu código *${c.code}* y acumulando recompensas. 🐸🎉`}
function queueMessage(client,type,text,dedupe=''){if(!client||!client.id)return;const items=getMessages();const key=dedupe||`${type}:${client.id}:${Date.now()}`;if(items.some(m=>m.dedupe===key))return;items.unshift({id:'MSG-'+Date.now()+'-'+Math.random().toString(36).slice(2,7),clientId:client.id,clientName:client.name,phone:client.phone,type,text,status:'pending',dedupe:key,createdAt:new Date().toISOString(),sentAt:null});setMessages(items)}
function refreshMessageStats(){const pending=getMessages().filter(m=>m.status==='pending').length;if($('statMessages'))$('statMessages').textContent=pending;if($('messagePendingCount'))$('messagePendingCount').textContent=pending;if($('statMembers'))$('statMembers').textContent=getLoyalty().length}
let messageFilter='pending',currentMessageId=null;
function renderMessages(){if(!$('messageList'))return;const all=getMessages(),items=all.filter(m=>m.status===messageFilter);$('messagePendingCount').textContent=all.filter(m=>m.status==='pending').length;$('showPendingMessages').className='btn '+(messageFilter==='pending'?'btn-primary':'btn-light');$('showSentMessages').className='btn '+(messageFilter==='sent'?'btn-primary':'btn-light');$('messageList').innerHTML=items.length?items.map(m=>`<div class="message-item"><div class="message-head"><div><span class="message-type ${m.type}">${messageTypeLabel(m.type)}</span><strong>${escapeHtml(m.clientName)}</strong><small>${escapeHtml(m.phone||'Sin teléfono')} · ${new Date(m.createdAt).toLocaleString('es-MX')}</small></div><span class="message-status ${m.status}">${m.status==='pending'?'Pendiente':'Enviado'}</span></div><div class="message-preview">${escapeHtml(m.text).replace(/\n/g,'<br>')}</div><div class="message-actions">${m.status==='pending'?`<button class="btn btn-light" onclick="editMessage('${m.id}')">Editar</button><button class="btn btn-whatsapp" onclick="sendMessageWhatsApp('${m.id}')">Enviar por WhatsApp</button><button class="btn btn-blue" onclick="markMessageSent('${m.id}')">Marcar enviado</button><button class="btn btn-danger-light" onclick="deleteMessage('${m.id}')">Eliminar</button>`:`<button class="btn btn-whatsapp" onclick="sendMessageWhatsApp('${m.id}',false)">Reenviar</button><button class="btn btn-danger-light" onclick="deleteMessage('${m.id}')">Eliminar</button>`}</div></div>`).join(''):'<div class="empty">No hay mensajes en esta bandeja.</div>';refreshMessageStats()}
window.editMessage=id=>{const m=getMessages().find(x=>x.id===id);if(!m)return;currentMessageId=id;$('messageEditRecipient').textContent=`Para: ${m.clientName} · ${m.phone}`;$('messageEditText').value=m.text;$('messageEditModal').classList.remove('hidden')}
window.markMessageSent=id=>{const items=getMessages(),i=items.findIndex(x=>x.id===id);if(i<0)return;items[i].status='sent';items[i].sentAt=new Date().toISOString();setMessages(items)}
window.deleteMessage=id=>{if(!confirm('¿Eliminar este mensaje?'))return;setMessages(getMessages().filter(x=>x.id!==id))}
window.sendMessageWhatsApp=(id,mark=true)=>{const m=getMessages().find(x=>x.id===id);if(!m)return;const phone=normalizedPhone(m.phone);window.open(`https://wa.me/${phone?'52'+phone:''}?text=${encodeURIComponent(m.text)}`,'_blank');if(mark)markMessageSent(id)}
$('showPendingMessages')?.addEventListener('click',()=>{messageFilter='pending';renderMessages()});$('showSentMessages')?.addEventListener('click',()=>{messageFilter='sent';renderMessages()});$('clearSentMessages')?.addEventListener('click',()=>{if(confirm('¿Eliminar el historial de mensajes enviados?'))setMessages(getMessages().filter(m=>m.status!=='sent'))});$('closeMessageEdit')?.addEventListener('click',()=>$('messageEditModal').classList.add('hidden'));$('saveMessageEdit')?.addEventListener('click',()=>{const items=getMessages(),i=items.findIndex(x=>x.id===currentMessageId);if(i<0)return;items[i].text=$('messageEditText').value.trim();setMessages(items);$('messageEditModal').classList.add('hidden')});

window.completeContractAndStamp=id=>{const contracts=getContracts(),i=contracts.findIndex(x=>x.id===id);if(i<0)return;if(!confirm('¿Marcar el servicio como realizado y agregar 1 Estrella Brinky al cliente?'))return;const d=contracts[i],c=ensureLoyaltyClient(d.clientName,d.clientPhone);const clients=getLoyalty(),ci=clients.findIndex(x=>x.id===c.id);clients[ci].stamps=Number(clients[ci].stamps||0)+1;clients[ci].totalRents=Number(clients[ci].totalRents||0)+1;clients[ci].totalSpent=Number(clients[ci].totalSpent||0)+Number(d.total||0);clients[ci].history.unshift({date:new Date().toISOString(),type:'stamp',text:`Estrella Brinky por contrato ${d.id}`,contractId:d.id});localStorage.setItem(LOYALTY_KEY,JSON.stringify(clients));queueMessage(clients[ci],rewardStatus(clients[ci])?'reward':'update',updateMessage(clients[ci],`Recibiste 1 Estrella Brinky por tu renta ${d.id}.`),`contract:${d.id}`);contracts[i]={...d,completed:true,completedAt:new Date().toISOString(),loyaltyClientId:c.id};setContracts(contracts);renderLoyalty();alert('Servicio realizado. Se agregó 1 Estrella Brinky y se creó un mensaje pendiente.')}
...
function updateCurrentLoyalty(delta,text,type='adjustment'){const items=getLoyalty(),i=items.findIndex(x=>x.id===currentLoyaltyId);if(i<0)return;items[i].stamps=Math.max(0,Number(items[i].stamps||0)+delta);items[i].history=items[i].history||[];items[i].history.unshift({date:new Date().toISOString(),type,text});setLoyalty(items);if(delta>0)queueMessage(items[i],rewardStatus(items[i])?'reward':'update',updateMessage(items[i],text));openLoyalty(currentLoyaltyId)}
$('loyaltyForm')?.addEventListener('submit',e=>{e.preventDefault();const phone=$('loyaltyPhone').value.trim(),key=normalizedPhone(phone),items=getLoyalty();if(items.some(c=>normalizedPhone(c.phone)===key&&key)){alert('Ya existe un cliente con ese teléfono.');return}const c={id:'LC-'+Date.now(),code:nextLoyaltyCode(),name:$('loyaltyName').value.trim(),phone,birthday:$('loyaltyBirthday').value,location:$('loyaltyLocation')?.value.trim()||'',notes:$('loyaltyNotes').value.trim(),stamps:1,totalRents:0,totalSpent:0,referrals:0,history:[{date:new Date().toISOString(),type:'welcome',text:'Estrella Brinky de bienvenida'}],createdAt:new Date().toISOString()};const ref=$('loyaltyReferrer').value.trim().toUpperCase();let referrer=null;if(ref){const ri=items.findIndex(x=>String(x.code).toUpperCase()===ref);if(ri>=0){items[ri].stamps=Number(items[ri].stamps||0)+1;items[ri].referrals=Number(items[ri].referrals||0)+1;items[ri].history.unshift({date:new Date().toISOString(),type:'referral',text:`Estrella Brinky por recomendar a ${c.name}`});c.history.unshift({date:new Date().toISOString(),type:'referral',text:`Registro mediante referencia ${ref}`});c.referredBy=items[ri].id;referrer=items[ri]}}items.unshift(c);setLoyalty(items);queueMessage(c,'welcome',welcomeMessage(c),`welcome:${c.id}`);if(referrer)queueMessage(referrer,'referral',referralMessage(referrer,c.name),`referral:${c.id}:${referrer.id}`);e.target.reset();alert(`Cliente registrado con 1 Estrella de Bienvenida. Código: ${c.code}
Se creó un mensaje pendiente de WhatsApp.`)})
...
refreshMessageStats();renderMessages();
